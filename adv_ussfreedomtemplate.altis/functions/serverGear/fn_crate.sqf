	...
	if !(_x isEqualTo objNull) then {
		...
		switch (true) do {
			...
			case (_par_customWeap == 2 || _par_customWeap == 3 || _par_customWeap == 4): {
				...
				_target addMagazineCargoGlobal ["rhsusf_10Rnd_762x51_m118_special_Mag",10];
		...
		if ( isClass(configFile >> "CfgMagazines" >> "AMP_Breaching_Charge_Mag") ) then {
			_target addMagazineCargoGlobal ["AMP_Breaching_Charge_Mag",5]
		};
		...
		_target addWeaponCargoGlobal ["BINOCULAR",5];
