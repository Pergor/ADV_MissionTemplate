...
if (worldName == "TANOA") then {
	_primaryweapon = ["arifle_MX_Black_F","arifle_MX_khk_F","arifle_MXC_Black_F","arifle_MXC_khk_F"];
...
_itemsBackpack = ["ToolKit","MineDetector","SatchelCharge_Remote_Mag","DemoCharge_Remote_Mag","DemoCharge_Remote_Mag"];
...
switch (_par_customUni) do {
	...
	case 13: {
		...
		if (_special isEqualTo "EOD") then {
			_vest = ["V_EOD_blue_F","V_EOD_coyote_F","V_EOD_olive_F"];
			_headgear = ["H_PASGT_basic_blue_F","H_PASGT_basic_olive_F"];
			_backpack = ["B_Messenger_Olive_F","B_Messenger_Black_F","B_Messenger_Coyote_F","B_Messenger_Gray_F"];
		};
	};
	case 14: {
		//TRYK Snow
