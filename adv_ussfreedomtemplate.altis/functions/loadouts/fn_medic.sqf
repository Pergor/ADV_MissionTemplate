...
switch (_par_customUni) do {
	...
	case 31: {
		...
		if (isClass(configFile >> "CfgPatches" >> "adv_retex")) then {
			_uniform = ["adv_retex_u_CombatUniform_wdl","adv_retex_u_CombatUniform_wdl_vest","adv_retex_u_CombatUniform_i_wdl_shortsleeve"];
			_headgear = ["H_HelmetIA_wdl"];
			_backpack = ["B_Kitbag_rgr_medic"];
		};
	};	
	default {};
};
...
if ( {[_player,_x] call adv_fnc_inGroup} count ["JUPITER","ZEUS"] > 0 || [_player,"command"] call adv_fnc_findInGroup ) then {
	_binocular = "Rangefinder";
	_androidDevice = true;
	_microDAGR = false;
	_ACE_MapTools = 1;
