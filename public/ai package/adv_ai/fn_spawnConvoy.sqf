 ...
 *
 * Example:
 * [spawnLogic,destinationLogic,["O_MRAP_02_hmg_F","O_Truck_02_transport_F","O_Truck_02_transport_F","O_Truck_02_transport_F","O_MRAP_02_hmg_F"],["O_Soldier_SL_F","O_Soldier_AR_F","O_Soldier_GL_F","O_Soldier_F","O_soldier_LAT_F","O_medic_F"],east,["NORMAL","SAFE","GREEN","COLUMN"],[stationLogic_1,stationLogic_2],50] call adv_ai_fnc_spawnConvoy;
 *
 * Public: Yes
 */
...
private _destination = [_targetLocation] call adv_ai_fnc_getPos;
//select starting pos depending on given parameter:
private _start = [_location] call adv_ai_fnc_getPos;
...
private _grp = [
	...
	,_vehicles
	,_side
] call adv_ai_fnc_spawnGroup;
[_grp,10] call adv_ai_fnc_setSafe;
_grp allowFleeing 0;
...
_returnWP setWaypointStatements ["true", "{deleteVehicle _x} forEach ([group _this ] call adv_ai_fnc_getGroupVehicles); {deleteVehicle _x} foreach thisList;"];
...
//spawn a group for each vehicle and move it in cargo positions:
{
	...
	[_grp_inf,10] call adv_ai_fnc_setSafe;
