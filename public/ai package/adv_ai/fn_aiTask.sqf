 ...
 *
 * Example:
 * ["spawnMarker",["O_Soldier_TL_F","O_Soldier_GL_F"],east,0,200] call adv_ai_fnc_aiTask;
 * or
 * ["spawnMarker",["O_Soldier_TL_F","O_Soldier_GL_F"],east,4,200,[attackLogic,50]] call adv_ai_fnc_aiTask;
 *
 * Public: Yes
 */
...
call {
	...
	if (_mode isEqualTo 2) exitWith {
		[_start, units _grp, _radius, true, true] call adv_ai_fnc_ZenOccupyHouse;
	};
	...
	if (_mode isEqualTo 4) exitWith {
		...
		if (_target isEqualTo [0,0,0]) then {
			...
			private _enemy = [_leader,5000] call adv_ai_fnc_findNearestEnemy;
