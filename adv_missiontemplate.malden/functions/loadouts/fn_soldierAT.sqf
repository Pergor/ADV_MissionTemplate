...
switch (_par_customWeap) do {
	...
	case 6: {
		...
		if ( _par_NVGs > 0 ) then { _attachments = ["CUP_acc_ANPEQ_2_grey"]; };
	...
	case 9: {
		...
		_optic = [""];
		if ( _par_NVGs > 0 ) then { _attachments = [""]; };
		_silencer = "";
