...

{
	...
	private _id = _x addAction [
		("<t color=""#FFFFFF"">" + ("Abwurf vorbereiten") + "</t>"), 
		{
			...
			if ( !(_respMarker isEqualTo "") && {_target distance (getMarkerPos _respMarker) > 200}) exitWith { (_target) removeAction _id; ["Zu weit vom Start entfernt",5] call adv_fnc_timedHint; };
