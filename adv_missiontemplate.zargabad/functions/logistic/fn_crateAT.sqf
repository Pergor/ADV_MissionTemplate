...

{
	...
	switch (true) do {
		...
		case (ADV_par_customWeap == 1): {
			//weapons
			_target addWeaponCargoGlobal ["BWA3_Pzf3_Loaded",4];
			_target addWeaponCargoGlobal ["BWA3_RGW90_Loaded",3];
		...
		case (ADV_par_customWeap == 5 || ADV_par_customWeap == 6 || ADV_par_customWeap == 7): {
			...
			//ammo
			if !(isClass(configFile >> "CfgPatches" >> "ace_disposable")) then { _target addMagazineCargoGlobal ["CUP_M136_M",3]; };
