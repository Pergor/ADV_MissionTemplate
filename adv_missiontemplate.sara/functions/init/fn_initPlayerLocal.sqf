...
if ( (missionNamespace getVariable ["ADV_par_ChooseLoad",1]) isEqualTo 1 ) then {
	{
		call {
			if ( ["opf_briefing",str _x] call BIS_fnc_inString ) exitWith {
				...
				_x addAction [("<t color='#FFFFFF' size='2' align='center'>" + ("Combat Patrol starten") + "</t>"), { [] remoteExec ["adv_fnc_cpinit",0] },nil,40,true,true,"","adv_par_headlessClient isEqualTo 3 && player == (leader (group player))",5];
			};
