...
if (true) exitWith {
	...
	if (ADV_par_Assets_cars == 0) then {
		if (str _vehicle in ADV_veh_light) then {
			[_vehicle] call ADV_fnc_disableVeh;
		};
	};
	if (ADV_par_Assets_heavy == 0) then {
		if (str _vehicle in ADV_veh_heavys) then {
			[_vehicle] call ADV_fnc_disableVeh;
		};
	};
	if (ADV_par_Assets_tanks == 0) then {
		if (str _vehicle in ADV_veh_tanks+ADV_veh_artys) then {
			[_vehicle] call ADV_fnc_disableVeh;
		};
	};
	if (ADV_par_Assets_air_helis == 0) then {
		if (str _vehicle in ADV_veh_helis) then {
			[_vehicle] call ADV_fnc_disableVeh;
		};
	};
	if (ADV_par_Assets_air_fixed == 0) then {
		if (str _vehicle in ADV_veh_fixedWing) then {
			[_vehicle] call ADV_fnc_disableVeh;
		};
	};
