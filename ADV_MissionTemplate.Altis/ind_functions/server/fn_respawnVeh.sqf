...
while {true} do {
	...
	if ((str _veh) in ADV_ind_veh_all) then {
		...
		if (_veh isKindOf "Helicopter" || _veh isKindOf "Ship") then {[_veh,false,false,2,false] call ADV_ind_fnc_vehicleLoad;};
		if (_veh isKindOf "Plane") then {[_veh,false,false,1,false] call ADV_ind_fnc_vehicleLoad;};
		if (_veh isKindOf "Tank") then {[_veh,false,false,2,false] call ADV_ind_fnc_vehicleLoad;};
		if (_veh isKindOf "Car") then {[_veh,false,false,1,false] call ADV_ind_fnc_vehicleLoad;};
		if (_veh isKindOf "Motorcycle") then {[_veh,false,false,0,false] call ADV_ind_fnc_vehicleLoad;};
	};
