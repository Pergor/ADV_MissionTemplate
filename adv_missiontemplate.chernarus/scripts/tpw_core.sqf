...
2 - Call it with 0 = [] execvm "scripts\tpw_core.sqf"; 
...
tpw_core_fnc_civs =
	...
	if (worldname in [
			...
			"tunba",
			"Kunduz"
		] && (isClass(configFile >> "CfgPatches" >> "lop_faction_tak_civ"))
	) then {
		_civstring = "LOP_Tak_Civ";
	};
	// TROPICAL
	if (worldname in [
			"lingor3",
			...
			"Porto",
			"Intro"
		] && (isClass(configFile >> "CfgPatches" >> "lop_faction_afr_civ")) 
	) then {
	...
	if (isClass(configFile >> "CfgPatches" >> "rds_a2port_cman")) then {
		if (worldname in 
			[
				"Altis",
				"Stratis",
