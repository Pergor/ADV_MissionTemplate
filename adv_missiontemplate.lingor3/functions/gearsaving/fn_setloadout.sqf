...
if (count _loadedMagazines>=3) then {
	{ 
		...
		[_target, _x, { (backpackContainer _target) addItemCargoGlobal [_x,1] }] call _add;
	...
	if(isClass(configFile>>"CfgWeapons">>_outfit)) then {			
		...
		if(loadUniform _target > 0) then {
			...
			{ 
				...
				[_target, _x, { (uniformContainer _target) addItemCargoGlobal [_x,1] }] call _add;
	...
	if(isClass(configFile>>"CfgWeapons">>_outfit)) then {
		...
		if(loadVest _target > 0) then {
			...
			{ 
				...
				[_target, _x, { (vestContainer _target) addItemCargoGlobal [_x,1] }] call _add;
