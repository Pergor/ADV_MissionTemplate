...
_goggles = "";
_unitTraits = [["medic",false],["engineer",false],["explosiveSpecialist",false],["UAVHacker",true],["camouflageCoef",1.0],["audibleCoef",1.0]];
...
switch (_par_customWeap) do {
	...
	case 3: {
		...
		_optic = ["rhsusf_acc_compm4","rhsusf_acc_eotech_552","rhsusf_acc_eotech_552","rhsusf_acc_ACOG_USMC","rhsusf_acc_ACOG3_USMC"];
		if ( _par_NVGs == 1 ) then { _attachments = ["rhsusf_acc_M952V"]; };
		if ( _par_NVGs == 2 ) then { _attachments = ["rhsusf_acc_anpeq15side_bk"]; };
		_silencer = "rhsusf_acc_rotex5_grey";
		_primaryweaponAmmo set [1,9];
...
switch (_par_customUni) do {
	...
	case 7: {
		...
		//_backPack = ["B_rhsusf_B_BACKPACK"];
		_itemsLink deleteAt (_itemsLink find "B_UavTerminal");
	...
	case 20: {
		//APEX NATO
		_uniform = ["U_B_T_Soldier_F","U_B_T_Soldier_SL_F"];
