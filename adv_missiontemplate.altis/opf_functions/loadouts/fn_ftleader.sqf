 ...
 *
 * Example:
 * [player] call adv_opf_fnc_ftLeader;
 *
 * Public: No
 */
...
_optic = ["optic_Arco","optic_MRCO","optic_MRCO"];
_attachments = [""];
...
switch (_par_opfUni) do {
	case 1: {
		...
		_vest = ["rhs_6b23_digi_6sh92_vog_headset","rhs_6b23_6sh92_vog_headset"];
	...
	case 3: {
		...
		_vest = ["rhs_6b23_ML_6sh92_vog_headset"];
...
switch ( toUpper ((str _player) select [4,4]) ) do {
	...
	default {
		switch true do {
			...
			case (toUpper ((str _player) select [4,9]) isEqualTo "ASSMORTAR"): { [_player,4] call ADV_fnc_CSW; _giveRiflemanRadio = false;_givePersonalRadio = true; };
			case (toUpper ((str _player) select [4,6]) isEqualTo "ASSTOW"): { [_player,6] call ADV_fnc_CSW; _giveRiflemanRadio = false;_givePersonalRadio = true; };
			default {};
		};
	};
};
