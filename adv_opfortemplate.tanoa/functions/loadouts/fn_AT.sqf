...
switch (ADV_par_customWeap) do {
	...
	case 3: {
		...
		if (ADV_par_optics > 0) then { _items pushBack "rhs_weap_optic_smaw"; };
	};
	case 4: {
		//RHS SOF
	...
	case 20: {
		...
		if !((toUpper worldname) in ADV_var_aridMaps) then { _launcher = "launch_B_Titan_short_tna_F"; };
	};
	default {};
};
switch (ADV_par_customUni) do {
	...
	case 4: {
		...
		_backpack = ["TFA_assault_FOL","TFA_assault_grn"];
	};
	case 5: {
		//TFA Desert
