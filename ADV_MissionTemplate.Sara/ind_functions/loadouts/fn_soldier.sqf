...
_vest = ["V_PlateCarrier1_blk","V_TacVestIR_blk","V_TacVest_blk","V_TacVest_oli","V_Chestrig_blk"];
...
switch (ADV_par_indWeap) do {
	...
	case 2: {
		//SELmods
		_primaryweapon = ["rhs_weap_m4_carryhandle","rhs_weap_m4a1_blockII","rhs_weap_m16a4_carryhandle","rhs_weap_m16a4_carryhandle","rhs_weap_m16a4_grip"];
		...
		//if (isClass(configFile >> "CfgPatches" >> "FHQ_Accessories")) then {_optic append [""];};
...
if ( [(str (_this select 0)),0,7] call BIS_fnc_trimString == "IND_ACSW" ) then { _binocular = "Rangefinder"; };
...
switch ( toUpper ([(str _player),0,7] call BIS_fnc_trimString) ) do {
	case "IND_CSW_": { [_player,1] call ADV_fnc_CSW; };
	case "IND_ACSW": { [_player,2] call ADV_fnc_CSW; };
	case "IND_MORT": { [_player,3] call ADV_fnc_CSW; };
	case "IND_AMOR": { [_player,4] call ADV_fnc_CSW; };
	case "IND_TOW_": { [_player,5] call ADV_fnc_CSW; };
